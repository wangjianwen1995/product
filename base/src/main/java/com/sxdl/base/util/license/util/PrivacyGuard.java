...
import com.sxdl.base.util.license.exception.PersistenceServiceException;
import com.sxdl.base.util.license.xml.GenericCertificate;
import com.sxdl.base.util.license.xml.PersistenceService;
...
public class PrivacyGuard {
	private static final String PBE_WITH_MD5_AND_DES = new ObfuscatedString(
			new long[] { 2860604316472308139L, 5030391952891038168L, -6110818099732428353L }).toString();
	...
	public void setCipherParam(CipherParam paramCipherParam) throws NullPointerException, IllegalPasswordException {
	...
	public byte[] cert2key(GenericCertificate paramGenericCertificate) throws Exception {
		ByteArrayOutputStream localByteArrayOutputStream = new ByteArrayOutputStream();
		GZIPOutputStream localGZIPOutputStream = new GZIPOutputStream(
				new CipherOutputStream(localByteArrayOutputStream, getCipher4Encryption()));
		try {
	...
	public GenericCertificate key2cert(byte[] paramArrayOfByte) throws Exception {
		GZIPInputStream localGZIPInputStream = new GZIPInputStream(
				new ByteArrayInputStream(getCipher4Decryption().doFinal(paramArrayOfByte)));
		try {
			GenericCertificate localGenericCertificate = (GenericCertificate) PersistenceService
					.load(localGZIPInputStream);
