...
//                    logger.error("错误的查询!", e);
    ...
    public PageInfo<T> queryPageListBufferOrderByTime(PageInfo<T> pageInfo, Object obj, String columnTime, String starTime, String endTime) {
        SaRequest request = SaHolder.getRequest();
        Integer pageNum = Integer.valueOf(request.getParam("pageNum", pageInfo.getPageNum() + ""));
        Integer pageSize = Integer.valueOf(request.getParam("pageSize", pageInfo.getPageSize() + ""));
        PageHelper.startPage(pageNum, pageSize);
        ...
        Class<? extends Object> class1 = obj.getClass();
        Example example = new Example(class1);
        example.setOrderByClause(columnTime+" DESC"); //ASC升序排列，desc为降序排列。
        Example.Criteria criteria = example.createCriteria();
        Field[] fields = class1.getDeclaredFields(); // 获取该类所有属性
        for (Field field : fields) {
            String name = field.getName();
...
//                    logger.error("错误的查询!", e);
    ...
    public Long selectCountWithSQL(String fromAndWhere) {
        ...
        List<Map<String, Object>> maps = selectSqlWithSQL(sql);
        return Long.parseLong(CollUtil.isNotEmpty(maps)?maps.get(0).get("cnt").toString():"0");
    }
    ...
    public Long selectCountWithSQL1(String column,String fromAndWhere) {
        String sql = SQLPackageUtil.getCountSQL1(column,fromAndWhere);
        List<Map<String, Object>> maps = selectSqlWithSQL(sql);
        return Long.parseLong(CollUtil.isNotEmpty(maps)?maps.get(0).get("cnt").toString():"0");
    }
}
