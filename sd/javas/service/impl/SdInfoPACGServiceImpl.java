...
import com.sxdl.sd.dao.dao1.SdInfoPACGDao;
...
@Service
@Transactional
public class SdInfoPACGServiceImpl extends BaseServiceImpl<SdInfoPACG> implements SdInfoPACGService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoPACGDao  sdInfoPACGDao;
    
    @Override
    public void insertOrUpdate(SdInfoPACG sdInfoPACG, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
            ...
            Integer id = sdInfoPACG.getId ();
            if (null != id && !"".equals ( id ) && id > 0) { 
sdInfoPACGDao.updateByPrimaryKeySelective ( sdInfoPACG );
            } else {
                //病人不存在  新增
sdInfoPACGDao.insertSelective (  sdInfoPACG);
            }
        }
    }}