...
import com.sxdl.sd.dao.dao1.SdInfoTNDao;
...
@Service
@Transactional
public class SdInfoTNServiceImpl extends BaseServiceImpl<SdInfoTN> implements SdInfoTNService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoTNDao  sdInfoTNDao;
    
    @Override
    public void insertOrUpdate(SdInfoTN sdInfoTN, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
            ...
            Integer id = sdInfoTN.getId ();
            if (null != id && !"".equals ( id ) && id > 0) { 
sdInfoTNDao.updateByPrimaryKeySelective ( sdInfoTN );
            } else {
                //病人不存在  新增
sdInfoTNDao.insertSelective (  sdInfoTN);
            }
        }
    }}