...
import com.sxdl.sd.dao.dao1.SdInfoOITDao;
...
@Service
@Transactional
public class SdInfoOITServiceImpl extends BaseServiceImpl<SdInfoOIT> implements SdInfoOITService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoOITDao  sdInfoOITDao;
    
    @Override
    public void insertOrUpdate(SdInfoOIT sdInfoOIT, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
            ...
            Integer id = sdInfoOIT.getId ();
            if (null != id && !"".equals ( id ) && id > 0) { 
sdInfoOITDao.updateByPrimaryKeySelective ( sdInfoOIT );
            } else {
                //病人不存在  新增
sdInfoOITDao.insertSelective (  sdInfoOIT);
            }
        }
    }}