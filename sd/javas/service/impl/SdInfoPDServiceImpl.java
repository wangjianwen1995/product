...
import com.sxdl.sd.dao.dao1.SdInfoPDDao;
...
@Service
@Transactional
public class SdInfoPDServiceImpl extends BaseServiceImpl<SdInfoPD> implements SdInfoPDService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoPDDao  sdInfoPDDao;
    
    @Override
    public void insertOrUpdate(SdInfoPD sdInfoPD, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
            ...
            Integer id = sdInfoPD.getId ();
            if (null != id && !"".equals ( id ) && id > 0) { 
sdInfoPDDao.updateByPrimaryKeySelective ( sdInfoPD );
            } else {
                //病人不存在  新增
sdInfoPDDao.insertSelective (  sdInfoPD);
            }
        }
    }}