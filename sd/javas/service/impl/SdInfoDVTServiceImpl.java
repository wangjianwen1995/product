...
import com.sxdl.sd.dao.dao1.SdInfoDVTDao;
...
@Service
@Transactional
public class SdInfoDVTServiceImpl extends BaseServiceImpl<SdInfoDVT> implements SdInfoDVTService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoDVTDao  sdInfoDVTDao;
    
    @Override
    public void insertOrUpdate(SdInfoDVT sdInfoDVT, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
            ...
            Integer id = sdInfoDVT.getId ();
            if (null != id && !"".equals ( id ) && id > 0) { 
sdInfoDVTDao.updateByPrimaryKeySelective ( sdInfoDVT );
            } else {
                //病人不存在  新增
sdInfoDVTDao.insertSelective (  sdInfoDVT);
            }
        }
    }}