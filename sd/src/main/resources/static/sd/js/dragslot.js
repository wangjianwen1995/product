  ...
  Dragslot.prototype = {
    ...
    _dragStart : function(e){
      ...
      if(dragItem[0] && dragItem[0].parentNode){
        offset_x = $(dragItem[0]).children('.move').offset().left;
        offset_y = $(dragItem[0]).children('.move').offset().top;
      ...
      this.dragElement.css({
        'left' : offset_x-e.offsetX,
        'top'  : clientY-e.offsetY
      });
    ...
    _dragMove : function(e){
      ...
      if(this.pointEl.hasClass(this.options.slotListClass)) {
        if(this.pointEl.children(this.options.slotItemClass).length == 0) {
          this.pointEl.append(this.placeholder);
          return;
        }
        this.pointEl = this.pointEl.children(this.options.slotItemClass).last();
        this.pointEl.after(this.placeholder);
      }
      else if (this.pointEl.closest('.' + this.options.slotHandlerClass).length || this.pointEl.closest('.' + this.options.slotItemClass).length) {
...
//			}
  ...
  $.fn.dragslot = function(options){
    instance = new Dragslot(this,options);
  }

  $.fn.doRefresh = function() {
    instance.init();
  }
