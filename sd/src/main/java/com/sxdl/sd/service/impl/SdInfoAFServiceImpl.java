...
import com.sxdl.sd.dao.dao1.SdInfoAFDao;
...
@Service
@Transactional
public class SdInfoAFServiceImpl extends BaseServiceImpl<SdInfoAF> implements SdInfoAFService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoAFDao sdInfoAFDao;

    @Override
    public void insertOrUpdate(SdInfoAF sdInfoAF, SdPatientInfo sdPatientInfo) {
        ...
        if (sdInfoAF != null) {
            Integer id = sdInfoAF.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoAF.setCM_0_2_4_2(format);
                sdInfoAFDao.updateByPrimaryKeySelective ( sdInfoAF );
            } else {
                //病人不存在  新增
                sdInfoAFDao.insertSelective ( sdInfoAF );
            }
        }
    }
}