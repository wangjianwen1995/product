...
@Service
@Transactional
public class SdInfoEARServiceImpl extends BaseServiceImpl<SdInfoARDS> implements SdInfoARDSService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoARDSDao sdInfoARDSDao;

    @Override
    public void insertOrUpdate(SdInfoARDS sdInfoARDS, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
           ...
           sdPatientInfo.setSubmit_time ( format );
            ...
            Integer id = sdInfoARDS.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoARDS.setCM_0_2_4_2(format);
                sdInfoARDSDao.updateByPrimaryKeySelective ( sdInfoARDS );
            } else {
                //病人不存在  新增
                sdInfoARDSDao.insertSelective (  sdInfoARDS);
            }
        }
    }}
