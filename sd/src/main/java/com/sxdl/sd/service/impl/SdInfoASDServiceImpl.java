...
import com.sxdl.sd.dao.dao1.SdInfoASDDao;
...
@Service
@Transactional
public class SdInfoASDServiceImpl extends BaseServiceImpl<SdInfoASD> implements SdInfoASDService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoASDDao  sdInfoASDDao;
    
    @Override
    public void insertOrUpdate(SdInfoASD sdInfoASD, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
           ...
           sdPatientInfo.setSubmit_time ( format );
            ...
            Integer id = sdInfoASD.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoASD.setCM_0_2_4_2(format);
sdInfoASDDao.updateByPrimaryKeySelective ( sdInfoASD );
            } else {
                //病人不存在  新增
sdInfoASDDao.insertSelective (  sdInfoASD);
            }
        }
    }}