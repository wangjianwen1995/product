...
@Service
@Transactional
public class SdInfoSAPServiceImpl extends BaseServiceImpl<SdInfoSAP> implements SdInfoSAPService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoSAPDao sdInfoSAPDao;

    @Override
    public void insertOrUpdate(SdInfoSAP sdInfoSAP, SdPatientInfo sdPatientInfo) {
        if (sdPatientInfo != null) {
            ...
            Integer id = sdInfoSAP.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoSAP.setCM_0_2_4_2(format);
                sdInfoSAPDao.updateByPrimaryKeySelective ( sdInfoSAP );
            } else {
                //病人不存在  新增
                sdInfoSAPDao.insertSelective (  sdInfoSAP);
            }
        }
    }}
