...
import com.sxdl.sd.dao.dao1.SdInfoCAPDao;
...
@Service
@Transactional
public class SdInfoCAPServiceImpl extends BaseServiceImpl<SdInfoCAP> implements SdInfoCAPService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoCAPDao  sdInfoCAPDao;
    
    @Override
    public void insertOrUpdate(SdInfoCAP sdInfoCAP, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
           ...
           sdPatientInfo.setSubmit_time ( format );
            ...
            Integer id = sdInfoCAP.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoCAP.setCM_0_2_4_2(format);
sdInfoCAPDao.updateByPrimaryKeySelective ( sdInfoCAP );
            } else {
                //病人不存在  新增
sdInfoCAPDao.insertSelective (  sdInfoCAP);
            }
        }
    }}