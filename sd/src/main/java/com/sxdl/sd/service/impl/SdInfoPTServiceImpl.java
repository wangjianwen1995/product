...
import com.sxdl.sd.dao.dao1.SdInfoPTDao;
...
@Service
@Transactional
public class SdInfoPTServiceImpl extends BaseServiceImpl<SdInfoPT> implements SdInfoPTService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoPTDao  sdInfoPTDao;
    
    @Override
    public void insertOrUpdate(SdInfoPT sdInfoPT, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
           ...
           sdPatientInfo.setSubmit_time ( format );
            ...
            Integer id = sdInfoPT.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoPT.setCM_0_2_4_2(format);
sdInfoPTDao.updateByPrimaryKeySelective ( sdInfoPT );
            } else {
                //病人不存在  新增
sdInfoPTDao.insertSelective (  sdInfoPT);
            }
        }
    }}