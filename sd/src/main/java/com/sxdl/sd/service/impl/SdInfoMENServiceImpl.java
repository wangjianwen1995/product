...
import com.sxdl.sd.dao.dao1.SdInfoMENDao;
...
@Service
@Transactional
public class SdInfoMENServiceImpl extends BaseServiceImpl<SdInfoMEN> implements SdInfoMENService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoMENDao  sdInfoMENDao;
    
    @Override
    public void insertOrUpdate(SdInfoMEN sdInfoMEN, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
           ...
           sdPatientInfo.setSubmit_time ( format );
            ...
            Integer id = sdInfoMEN.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoMEN.setCM_0_2_4_2(format);
sdInfoMENDao.updateByPrimaryKeySelective ( sdInfoMEN );
            } else {
                //病人不存在  新增
sdInfoMENDao.insertSelective (  sdInfoMEN);
            }
        }
    }}