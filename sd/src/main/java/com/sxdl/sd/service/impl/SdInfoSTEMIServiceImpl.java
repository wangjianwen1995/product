...
import com.sxdl.sd.dao.dao1.SdInfoSTEMIDao;
...
@Service
@Transactional
public class SdInfoSTEMIServiceImpl extends BaseServiceImpl<SdInfoSTEMI> implements SdInfoSTEMIService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoSTEMIDao sdInfoSTEMIDao;

    @Override
    public void insertOrUpdate(SdInfoSTEMI sdInfoSTEMI, SdPatientInfo sdPatientInfo) {
        ...
        if (sdInfoSTEMI != null) {
            Integer id = sdInfoSTEMI.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoSTEMI.setCM_0_2_4_2(format);
                sdInfoSTEMIDao.updateByPrimaryKeySelective ( sdInfoSTEMI );
            } else {
                //病人不存在  新增
                sdInfoSTEMIDao.insertSelective ( sdInfoSTEMI );
            }
        }
    }
}