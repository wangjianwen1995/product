...
import com.sxdl.sd.dao.dao1.SdInfoPIPDao;
...
@Service
@Transactional
public class SdInfoPIPServiceImpl extends BaseServiceImpl<SdInfoPIP> implements SdInfoPIPService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoPIPDao  sdInfoPIPDao;
    
    @Override
    public void insertOrUpdate(SdInfoPIP sdInfoPIP, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
           ...
           sdPatientInfo.setSubmit_time ( format );
            ...
            Integer id = sdInfoPIP.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoPIP.setCM_0_2_4_2(format);
sdInfoPIPDao.updateByPrimaryKeySelective ( sdInfoPIP );
            } else {
                //病人不存在  新增
sdInfoPIPDao.insertSelective (  sdInfoPIP);
            }
        }
    }}