...
import com.sxdl.sd.dao.dao1.SdInfoTSCCDao;
...
@Service
@Transactional
public class SdInfoTSCCServiceImpl extends BaseServiceImpl<SdInfoTSCC> implements SdInfoTSCCService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoTSCCDao  sdInfoTSCCDao;
    
    @Override
    public void insertOrUpdate(SdInfoTSCC sdInfoTSCC, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
           ...
           sdPatientInfo.setSubmit_time ( format );
            ...
            Integer id = sdInfoTSCC.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoTSCC.setCM_0_2_4_2(format);
sdInfoTSCCDao.updateByPrimaryKeySelective ( sdInfoTSCC );
            } else {
                //病人不存在  新增
sdInfoTSCCDao.insertSelective (  sdInfoTSCC);
            }
        }
    }}