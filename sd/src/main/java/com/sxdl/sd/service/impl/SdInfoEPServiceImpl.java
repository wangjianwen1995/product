...
import com.sxdl.sd.dao.dao1.SdInfoEPDao;
...
@Service
@Transactional
public class SdInfoEPServiceImpl extends BaseServiceImpl<SdInfoEP> implements SdInfoEPService {
    @Autowired
    SdPatientInfoDao sdPatientInfoDao;
    @Autowired
    SdInfoEPDao  sdInfoEPDao;
    
    @Override
    public void insertOrUpdate(SdInfoEP sdInfoEP, SdPatientInfo sdPatientInfo) {
       if (sdPatientInfo != null) {
           ...
           sdPatientInfo.setSubmit_time ( format );
            ...
            Integer id = sdInfoEP.getId ();
            if (null != id && !"".equals ( id ) && id > 0) {
                String format = sdPatientInfo.getCy_time();
                format=format.substring(0,16);
                sdInfoEP.setCM_0_2_4_2(format);
sdInfoEPDao.updateByPrimaryKeySelective ( sdInfoEP );
            } else {
                //病人不存在  新增
sdInfoEPDao.insertSelective (  sdInfoEP);
            }
        }
    }}