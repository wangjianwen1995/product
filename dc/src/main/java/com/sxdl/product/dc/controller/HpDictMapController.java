...
import com.sxdl.product.dc.service.HpDictMapService;
...
public class HpDictMapController {
    ...
    public ResultUtil findMappingColumn(PageInfo pageInfo, @RequestParam(value = "name", defaultValue = "") String name) throws Exception {
        ...
        List<DcColumn> columns = columnService.findMappingColumn(name);
    ...
    @ApiOperation(value = "对照字典数据下拉框")
    @GetMapping("/findDict")
    public ResultUtil findDict() throws Exception {
        List<DcColumn> columns = (List<DcColumn>)ApplicationRunnerImpl.contextMap.get("dictMap");
    ...
    public ResultUtil findReportTempColumn(PageInfo pageInfo, @RequestParam(value = "name", defaultValue = "") String name) throws Exception {
        Map<String, Object> listPage = null;
        List<DcColumn> columns = columnService.findReportTempColumn(name);
        listPage = PageList.getListPage(pageInfo.getPageNum(), pageInfo.getPageSize(), columns);
        return ResultUtil.success(listPage);
    }
    ...
    public ResultUtil loadMappingData(@RequestParam(value = "his_code", required = false, defaultValue = "") String his_code,
        ...
        hpDictMaps = mapService.findDataByHisCodeAndColId(his_code,his_name, column_id);
    ...
    @PostMapping("/save")
    public ResultUtil save(@RequestBody DcColumn hpColumn) throws Exception {
        mapService.save(hpColumn);
        return ResultUtil.success("操作成功");
    }


    @ApiOperation(value = "查询上报平台字典")
    @GetMapping("/findReport")
    public ResultUtil findReport() throws Exception {
        String sql = "select * from drplus_platform_detailed where is_qy=1 ";
        List<Map<String, Object>> maps = mapService.selectSqlWithSQL(sql);
