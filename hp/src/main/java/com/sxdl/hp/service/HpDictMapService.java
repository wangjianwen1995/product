...
import com.sxdl.hp.dao.dao1.HpDictMapDao;
...
import com.sxdl.hp.util.ZipUtils;
...
public class HpDictMapService extends BaseServiceImpl<HpDictMap> {
    ...
    @Autowired
    HpColumnDao hpColumnDao;
    HpHospiatlInfo hi;
    @Autowired
    private HpDictMapDao dictMapDao;
    @Autowired
    YmlUtil ymlUtil;
    public List<HpDictMap> findDataByHisCodeAndColId(String his_code, String col_id){
...
//    }
    ...
    public Integer save(HpColumn hpColumn) {
    ...
    public void exportData(String hisCode, String type, HttpServletResponse response) throws Exception {
        ...
        hi = (HpHospiatlInfo) HpApplicationRunnerImpl.contextMap.get("hpHospiatlInfo");
     ...
     * @return
     */
    StringBuilder getDMLInsert(List<HpDictMap> datas, StringBuilder sb)  {
...
//        System.out.println(maps);
     ...
     * @return
     */
    public ResultUtil autoMap(String name, String hisCode) {
        ...
        List<String> instanceDatasStrs = hpColumnDao.findInstanceData(tname, instanceName);//查接口表中该字段distinct的数据
        ...
        HpDictMap dm = new HpDictMap(), nowDictMap = null;
        ...
        Map<Integer, List<SysDictVal>> dicts = (Map<Integer, List<SysDictVal>>) HpApplicationRunnerImpl.contextMap.get("baMap");
        List<HpColumn> mappingColumn = hpColumnDao.findMappingColumn(name);
        ...
        HpColumn column = new HpColumn();//系统中配置的字段对象
        if (CollUtil.isNotEmpty(mappingColumn)) {
            column = mappingColumn.get(0);//如果结果不空应该只有一条
            Integer dictTid = column.getDict_table_id();
    ...
    HpDictMap extractedDictmap(HpDictMap dm, HpColumn column, SysDictVal dv)  {
        dm.setDict_talbe_id(column.getDict_table_id());
        dm.setHp_table_id(column.getTable_id());
        dm.setHp_table_name(column.getTable_name());
        dm.setHp_column_id(column.getId());
        dm.setHp_column_name(column.getName());
     ...
     * @return
     */
    private ResultUtil preCheck(String name, String tname)  {
        //检查库中是否有表
        List<Map<String, Object>> maps = hpColumnDao.selectSqlWithSQL(SQLPackageUtil.generateIfExists(tname, " table "));
        if (CollUtil.isEmpty(maps)) {
            return ResultUtil.error("当前环境中dc没有接口表,请检查相关配置!");
        }
        String columns = hpColumnDao.getColumnsFromOneTable(tname);//查询表的字段列表
     ...
     * @return
     */
    String getMatchedTnC(String name, String tname)  {
