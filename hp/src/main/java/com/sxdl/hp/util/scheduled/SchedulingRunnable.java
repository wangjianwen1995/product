...
//     */
    public SchedulingRunnable(String beanName, String methodName, String template) {
        this.beanName = beanName;
        this.methodName = methodName;
...
//        logger.info("定时任务开始执行 - bean：{}，方法：{}，参数：{}", beanName, methodName, template);
        ...
        try {
            ...
            System.out.println(String.format("定时任务执行异常 - bean：%s，方法：%s，参数：%s ", beanName, methodName, template));
//            logger.error(String.format("定时任务执行异常 - bean：%s，方法：%s，参数：%s ", beanName, methodName, template), ex);
        }

        long times = System.currentTimeMillis() - startTime;
        System.out.println(String.format("定时任务执行结束 - bean：%s，方法：%s，参数：%s ", beanName, methodName, template));
